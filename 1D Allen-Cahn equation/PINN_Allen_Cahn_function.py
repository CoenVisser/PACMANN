...
def PINN_Allen_Cahn(train_number, 
    ...
    if collocation_type == "Uniform":
        data = dde.data.TimePDE(
            geomtime, pde, [], num_domain=amount_collocation_points, num_boundary=80, num_initial=160, num_test=10000, train_distribution="uniform"
            )
    elif collocation_type == "RAR" or collocation_type == "RAR_D":
        data = dde.data.TimePDE(
            geomtime, pde, [], num_domain=amount_collocation_points // 2, num_boundary=80, num_initial=160, num_test=10000
            )
    else:
        data = dde.data.TimePDE(
            geomtime, pde, [], num_domain=amount_collocation_points, num_boundary=80, num_initial=160, num_test=10000
            )
