...
import deepxde.backend as pt


def PINN_Allen_Cahn(train_number, 
                    random_seed_input               = True,
                    number_collocation_points_input = 2500,
    ...
    def output_transform(x, y):
        return x[:, 0:1]**2 * pt.cos(np.pi * x[:, 0:1]) + x[:, 1:2] * (1 - x[:, 0:1]**2) * y

    if collocation_type == "Uniform":
        data = dde.data.TimePDE(
            geomtime, pde, [], num_domain=number_collocation_points, num_boundary=80, num_initial=160, num_test=10000, train_distribution="uniform"
            )
    elif collocation_type == "RAR" or collocation_type == "RAR_D":
        data = dde.data.TimePDE(
            geomtime, pde, [], num_domain=number_collocation_points // 2, num_boundary=80, num_initial=160, num_test=10000
            )
    else:
        data = dde.data.TimePDE(
            geomtime, pde, [], num_domain=number_collocation_points, num_boundary=80, num_initial=160, num_test=10000
            )
    ...
    if save_collocation_points:
        with open('resampled_points.txt', 'w') as f:
            ...
            """
            f.write(f'0:\n')
            np.savetxt(f, model.data.train_x[240:, :])
    ...
    class Gradient_ascent(dde.callbacks.Callback):
        ...
        def on_epoch_end(self):
            ...
            if self.first_resample:
                collocation_points = self.model.data.train_x[240:, :]
...
if __name__ == "__main__":
    PINN_Allen_Cahn(train_number=0)